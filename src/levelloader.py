...
class LevelLoader(object):
    ...
    def __init__(self, level_file_name):
        ...
        for i, line in enumerate(raw_level_file_lines):
            if not line.strip():
                reason = "blank line found in file %s." % self.level_file_name
                raise MalformedLevelFileError(reason)
    ...
    def __set_level_symbols(self, level_symbol_lines):
        ...
        for line in level_symbol_lines:
            ...
            if name not in const.LEVEL_SYMBOL_TYPES:
                reason = (
                    "unrecognized symbol type '%s' found in file '%s'. "
            ...
            for key in level_sym:
                if name == key:
                    reason = (
                        "symbol type '%s' defined more than once in file "
                        "'%s'." % (
                            name,
                            self.level_file_name,
                        )
                ...
                if symbol == level_sym[key]:
                    reason = (
                        "symbol '%s' used for more than one symbol type in "
                        "file '%s'." % (
                            symbol,
                            self.level_file_name,
                        ),
        ...
        for line in self.level_file_lines:
            if line[0].isalpha():
                if line in level_names:
                    reason = (
                        "duplicate level name '%s' found in file '%s'." % (
                            line,
                            self.level_file_name,
                        )
                    )
    ...
    def get_level(self, disp, name=None):
        if name is not None:
            ...
            chosen_level_name = disp.level_prompt(
                self.level_names,
                self.level_file_name,
            )
        ...
        if (choice_number + 1) == len(self.level_names):
            ...
            end = self.level_file_lines.index(
                self.level_names[choice_number+1]
            )
    ...
    def __exception_if_level_unplayable(self):
        ...
        if players != 1:
            reason = (
                "level '%s' in file '%s' does not have exactly one player "
                "'%s'." % (
                    self.chosen_level_name,
                    self.level_file_name,
        ...
        if boulders < pits:
            reason = (
                "level '%s' in file '%s' does not have enough boulders (%s) "
                "to fill the pits '%s'." % (
                    self.chosen_level_name,
                    self.level_file_name,
