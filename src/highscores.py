...
class HighScores(object):
    ...
    def __init__(self,
        ...
        try:
            ...
            reason = "could not open high score file '%s'." % self.hs_file_name
    ...
    def save_high_scores(self, hs_file_name=None):
        ...
        temp_backup_file_name = os.path.join(
            os.path.split(hs_file_name)[0],
            'temp' + timestamp + const.HIGH_SCORE_EXTENSION,
        )
        ...
        try:
            ...
            reason = (
                "could not create backup for high score file '%s' before"
                "saving high scores. Current high score file is preserved." %
                hs_file_name
            )
            raise HighScoreFileHandlingError(reason)
        ...
        try:
            ...
            if not os.path.isfile(hs_file_name):
                reason = (
                    "could not create high score file '%s'." % hs_file_name
                )
            else:
                reason = (
                    "could not open high score file '%s' for writing." %
                    hs_file_name
                )
            try:
                ...
                reason += (
                    " Also could not delete high score backup file '%s'." %
        ...
        try:
            pickle.dump(self.scores, hs_file)
                ...
                IndexError):
            reason = (
                "could not save current high score data to file '%s'. Your "
                "current high score data can be restored from file '%s'." % (
                    hs_file_name,
                    temp_backup_file_name,
                )
            )
        ...
        try:
            ...
            reason = (
                "could not delete high score backup file '%s'." %
                temp_backup_file_name
            )
            raise HighScoreFileHandlingError(reason)
